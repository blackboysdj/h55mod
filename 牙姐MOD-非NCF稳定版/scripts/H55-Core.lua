...
H55_CI_Skeleton_Karissa = 0;
H55_Town_Skeleton_Karissa = 0;
...
function H55_KarissaCarryArmy(player, hero, ci)
	local s_hero = 'Karissa';
	if hero == s_hero then
		if H55_CI_Skeleton_Karissa ~= ci then
			H55_CI_Skeleton_Karissa = ci;
			if GetSavedCombatArmyHero(ci, 1) == s_hero then
				local iLevel = GetHeroLevel(s_hero);
				local iCoef = 0.02 + iLevel * 0.002;
				local iCountStacks = GetSavedCombatArmyCreaturesCount(ci, 0);
				local iCountTotalCreatureDeath = 0;
				for i = 0, iCountStacks - 1, 1 do
					local iCreature, iCountCreature, iCountCreatureDeath = GetSavedCombatArmyCreatureInfo(ci, 0, i);
					iCountTotalCreatureDeath = iCountTotalCreatureDeath + iCountCreatureDeath;
				end;
				print('iCountTotalCreatureDeath: '..iCountTotalCreatureDeath);
				if iCountTotalCreatureDeath > 0 and H55_Town_Skeleton_Karissa ~= 0 and H55_GetTownRace(H55_Town_Skeleton_Karissa) == 4 then
					local townrace = H55_GetTownRace(H55_Town_Skeleton_Karissa);
					local town = H55_Town_Skeleton_Karissa;
					if GetTownBuildingLevel(town, TOWN_BUILDING_DWELLING_1) >= 1 then
						print(GetObjectDwellingCreatures(town, H55_Creatures[townrace][1][1]));
						print(iCoef);
						print(H55_Ceil(iCoef * iCountTotalCreatureDeath));
						SetObjectDwellingCreatures(town, H55_Creatures[townrace][1][1], GetObjectDwellingCreatures(town, H55_Creatures[townrace][1][1]) + (H55_Ceil(iCoef * iCountTotalCreatureDeath)));
						print(GetObjectDwellingCreatures(town, H55_Creatures[townrace][1][1]));
					end;
				end;
			end;
...
function H55_WeeklyEvents(player)
	...
	if heroes~=nil then
		...
		-- end;
			...
			if contains(heroes, "Jeddite") ~= nil then
				H55_WeeklyReinforce("Jeddite", player, 5, STAT_KNOWLEDGE, -1, -1);
			end;
...
function H55_ContinuesEvent(player)
	...
	if heroes ~= nil and H55_IsThisAIPlayer(player) ~= 1 then
		if H55_length_playerheroes[player] ~= length(heroes) then
			...
			--heaven
				...
				H55_fun_start_creatures_bonus(heroes, "Jeddite", 9, 1);
	...
	if heroes~=nil then
		...
		for i,hero in heroes do
			...
			H55_KarissaCarryArmy(player,hero,ci);
...
-- begin 英雄自定义主动技能1~3
	...
	function TTH_HeroCustomAbility3(strHero)
		...
		if strHero == "Karissa" then
			TTH_HeroCustomAbility_Karissa(strHero);
		end;
...
-- by 牙姐 2018-10-22 19:08:10
-- begin 卡里萨指定复活城镇技能
	function TTH_HeroCustomAbility_Karissa(strHero)
		print("---------------------------------------------------------------------");
		print("TTH "..strHero.." Event begin");
		local iPlayer = GetObjectOwner(strHero);
		...
		if objTown == nil then
			QuestionBoxForPlayers(GetPlayerFilter(iPlayer), "/Text/Game/Scripts/HeroCustomAbility/Karissa/FailureNotAtGate.txt", "" ,"");
		else
			QuestionBoxForPlayers(GetPlayerFilter(iPlayer),	"/Text/Game/Scripts/HeroCustomAbility/Karissa/Success.txt", "TTH_HeroCustomAbility_Karissa_Callback('"..strHero.."','"..objTown.."')" , "");
			return nil;
		end;
		print("TTH "..strHero.." Event end");
		print("---------------------------------------------------------------------");
	end;
	function TTH_HeroCustomAbility_Karissa_Callback(strHero, objTown)
		H55_Town_Skeleton_Karissa = objTown
		print(objTown.." has been appointed");
		ControlHeroCustomAbility(strHero, CUSTOM_ABILITY_3, CUSTOM_ABILITY_DISABLED);
		print(strHero.." custom ability3 has been used");
	end;
