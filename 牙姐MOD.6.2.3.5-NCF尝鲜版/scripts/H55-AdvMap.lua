...
function TTH_BankReward(strHero, iLevel)
	...
	elseif iLevel == 1 then
		TTH_Reward_Gold(strHero, H55_AbsoluteWeek + 4);
	end;
end;
-- end
...
function TTH_BankRewardAI(strHero, iLevel)
	...
	elseif iLevel == 1 then
		if iRandom == 0 then
			iRewardParam = H55_AbsoluteWeek + 4;
		elseif iRandom == 1 then
			iRewardParam = 2;
		elseif iRandom == 2 then
			iRewardParam = 1;
...
function TTH_Reward_Spell(strHero, iRewardParam)
	...
	if strClass == "Barbarian" then
		local iShoutLevel = 1;
		if iHeroLevel >= 2 and iHeroLevel < 6 then
			iShoutLevel = 1;
		elseif iHeroLevel >= 6 and iHeroLevel < 11 then
			iShoutLevel = 2;
		elseif iHeroLevel >= 11 then
			iShoutLevel = 3;
		end;
		if TTH_Reward_Shout_Loop(iPlayer, strHero, "WARCRY", iShoutLevel, iRewardParam) == 1 then
			iCountLearn = 1;
		end;
	else
		local iSpellLevelLight = GetHeroSkillMastery(strHero, SKILL_LIGHT_MAGIC);
		if TTH_Reward_Spell_Loop(iPlayer, strHero, "LIGHT", iSpellLevelLight + 2, iRewardParam) == 1 then
			iCountLearn = 1;
		end;
		if iCountLearn == 0 then
			local iSpellLevelDark = GetHeroSkillMastery(strHero, SKILL_DARK_MAGIC);
			if TTH_Reward_Spell_Loop(iPlayer, strHero, "DARK", iSpellLevelDark + 2, iRewardParam) == 1 then
				iCountLearn = 1;
			end;
		end;
		if iCountLearn == 0 then
			local iSpellLevelSummoning = GetHeroSkillMastery(strHero, SKILL_SUMMONING_MAGIC);
			if TTH_Reward_Spell_Loop(iPlayer, strHero, "SUMMONING", iSpellLevelSummoning + 2, iRewardParam) == 1 then
				iCountLearn = 1;
			end;
		end;
		if iCountLearn == 0 then
			local iSpellLevelDestructive = GetHeroSkillMastery(strHero, SKILL_DESTRUCTIVE_MAGIC);
			if TTH_Reward_Spell_Loop(iPlayer, strHero, "DESTRUCTIVE", iSpellLevelDestructive + 2, iRewardParam) == 1 then
				iCountLearn = 1;
			end;
		end;
		if iCountLearn == 0 then
			local iSpellLevelRune = GetHeroSkillMastery(strHero, HERO_SKILL_RUNELORE);
			if iSpellLevelRune > 0 then
				if TTH_Reward_Spell_Loop(iPlayer, strHero, "RUNE", iSpellLevelRune, iRewardParam) == 1 then
					iCountLearn = 1;
				end;
			end;
		end;
	end;
	if iCountLearn == 1 then
		GiveExp(strHero, iHeroLevel * 1000);
	else
		GiveExp(strHero, iHeroLevel * 10000);
		if H55_IsThisAIPlayer(iPlayer) ~= 1 then
			ShowFlyingSign("/Text/Game/Scripts/BankReward/RewardSpell/TemplateNoMore.txt", strHero, iPlayer, 5);
			return nil;
		end;
	end;
end;
