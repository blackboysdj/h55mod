...
function H55_GrailTouch(s_hero, grail)
	...
	if H55_IsThisAIPlayer(player) ~= 1 then
		if HasHeroSkill(s_hero, KNIGHT_FEAT_GRAIL_VISION) ~= nil then
			...
			if i_rest == 1 then
				local i_movement = H55_Round(H55_GetHeroMovement(s_hero) / 4);
				...
				local i_remain_mana = H55_Round(i_mana_total / 4);
...
function H55_GetBankDifMultiplier()
	...
	if H55_BankTest == 0 then
		...
		weeks = (H55_TestDay/7)+1;
		normal = 1+((H55_TestDay/7)*(0.03+add));
		hard = 1+((H55_TestDay/7)*(0.04+add));
		heroic = 1+((H55_TestDay/7)*(0.05+add));
		impossible = 1+((H55_TestDay/7)*(0.06+add));
		reduction = 1;
		if H55_Difficulty == 0 then
			if H55_TestDay < 28 then reduction = 0.7 end;
			if H55_TestDay >= 28 and H55_TestDay < 56 then reduction = 0.8 end;
			if H55_TestDay >= 56 and H55_TestDay < 84 then reduction = 0.9 end;
		else
			if H55_TestDay < 28 then reduction = 0.8 end;
			if H55_TestDay >= 28 and H55_TestDay < 56 then reduction = 0.9 end;	
		end;
		if H55_GovernanceEnabled == 1 then reduction = reduction + 0.05 end;
		if H55_Difficulty == 3 then
			multiplier = H55_BanksDifficulty*reduction*(impossible*weeks);
...
function H55_WagonVisit(hero,bank)
	local player = GetObjectOwner(hero);
	if H55_GetLastVisited(bank) > 6 and H55_GetPlayerLastVisited(player,bank) > 13 then
...
function H55_OrcTunnelVisit(hero,bank)
	...
	if H55_GetLastVisited(bank) > 6 and H55_GetPlayerLastVisited(player,bank) > 13 then
		...
		if H55_IsThisAIPlayer(player) == 1 then
			...
			if H55_Day > 42 and H55_Day <= 112 then
				if combat02 == 1 or combat02 == 2 or combat02 == 3 then
